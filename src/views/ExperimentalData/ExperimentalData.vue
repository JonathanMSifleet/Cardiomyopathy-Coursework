  ...
  <PageWrapper>
    <MDBCard text="center" :class="[$style.MainCard]">
      <MDBCardBody :class="$style.CardBody">
        <MDBCardTitle :class="[[$style.CardTitle], 'mt-4']">Gene Mutation Data</MDBCardTitle>
        <MDBCardText>
          <p
            :class="[[$style.UploadFileInstructions], 'mt-5']"
          >Upload a file containing experimental data:</p>
          <div :class="$style.FileUploadWrapper">
            <FileUpload />
          </div>
          <p :class="[[$style.ManualDataInstructions], 'mt-5']">Or, insert it manually:</p>
          <form>
  ...
  setup() {
    ...
    const submitExperimentalData = async () => {
      if (
        ...
        alert("Gender must me 'male' or 'female'");
        return;
      }
